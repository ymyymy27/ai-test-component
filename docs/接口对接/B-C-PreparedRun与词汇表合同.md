...
需要强调：C 包的整体结构是**正确的**，`evidence_level: EvidenceLevelFact | None`（只有完整验证打分）与
`tier` 使用的枚举值都与 B 包一致。本文档只处理剩余的不一致。
...
| --- | --- | --- | --- | --- | --- | --- |
| **C-01** | `conclusion_ceiling` | 夹具取值为 `"full"` | 只能取 `partial` / `passable` | **取值错误。** `conclusion_ceiling` 是"结论上限"，由档位派生：quick/on_demand → `partial`，full → `passable`。写成 `"full"` 会让 D 无法判断"这次能不能判完整通过"，且因为是裸 `str`，**校验不会报错，错误会静默传播** | 夹具改为 `"passable"`；字段改为枚举 | C 改夹具，B 供枚举 |
...
> **C-01 是本文档最重要的一条。** 它是本次对接唯一一处**语义错误**，而不是风格问题。
...
> 改动是**向后兼容**的：`"planned"` 与 `"partial"/"passable"` 都是合法字符串值，只有 `"full"` 这个错误取值会被新枚举拒绝——
> 这正是我们想要的效果。
...
**B 的 `PreparedRun` 尚未定稿。** 上述字段名是 B 的草案，确认后以 `docs/接口对接/` 的下一版为准。
在 `PreparedRun` 冻结前，C 可继续使用夹具，**不必等待**。
...
**两者不可合并、不可互相替代。** 前者证明"我打算用这些来源"，后者证明"实际加载的就是这些"。
...
7. **B 侧问题**：`PreparedRun` 定稿后，你希望以什么形式接收——Python `Protocol`、Pydantic 合同，还是仅 JSON 夹具？
